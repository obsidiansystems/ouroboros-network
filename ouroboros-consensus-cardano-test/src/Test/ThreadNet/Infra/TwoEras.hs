...
genTestConfig k (EpochSize byronE, EpochSize shelleyE) = do
    initSeed <- arbitrary

    numSlots <- do
      let wiggle = min byronE (2 * maxRollbacks k)
...
secondEraOverlaySlots ::
  ...
  -> EpochSize
  -> Set SlotNo
secondEraOverlaySlots numSlots (NumSlots numByronSlots) d epochSizeShelley =
    Set.filter (< SlotNo t) $
    Set.unions $
...
tabulatePartitionPosition (NumSlots numByronSlots) part transitions =
  ...
  where
    ...
    inclByron   =
        dur > 0 && (not transitions || firstSlotIn    <= numByronSlots)
    inclShelley =
        dur > 0 && (transitions     && firstSlotAfter >= numByronSlots)
